...
code and save it as \file{hello.c}.
...
Lets now analyze the anatomy of this simple first C program. The very first
line, \verb|#include <stdio.h>| tells the compiler that we would like to use
the \ident{stdio} library. A library is a set of useful, reusable routines. In
this case, the \ident{stdio} library contains routines for interfacing with
...
What exactly a \ident{statement} is will be a little vague, but it will
